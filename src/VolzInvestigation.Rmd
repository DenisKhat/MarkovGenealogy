...
#### Likelihood Function?
```{R}
data <- read.csv("../simulation_results/likelihood_table_volz.csv")
print(data)
...
ggplot(data, aes(x=beta, y=gamma)) +
  ...
  geom_point(aes(x=0.7, y=0.3),color="seagreen") +
    annotate('text', x=0.7, y=0.3 + 0.05, color="seagreen", label="\u0398") +
    labs(
      title=paste("Simulation", 800 ," Volz Simulated LogLikelihood Heatmap"),
      x="beta",
      y="gamma",
      fill="Log Likelihood"
    )
...
```{R}
ss_data <- read.csv(file="../sim_study_volz.csv")
ss_data$X <- NULL
ss_data
means_data <- data.frame(beta=c(), gamma=c(), mean_likelihood=c())
for (b in seq(0.1, 1, by=0.1)){
  for (g in seq(0.1, 1, by=0.1)){
    mean_likelihood <- 
  }
}
```