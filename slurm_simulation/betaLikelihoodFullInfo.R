...
full_knowledge_estimation <- function(data, alpha=0.05){
  ...
  lower_wilks = uniroot(curve, lower = 0.05, upper=MLE$par)$root
  upper_wilks = uniroot(curve, lower = MLE$par, upper = 1)$root
  if (params$beta < lower_wilks || params$beta > upper_wilks){
    in_interval <- 0
  }
  
  return(list(mle=MLE$par, in_interval=in_interval))
}
...
output_table <- data.frame(estimate=estimate, in_inteval=in_interval)
write.csv(output_table,file=paste0("full_knowledge_estimate_",job_id,".csv"),row.names=F)
